...
router.get("/notes/getall", async (context) => {
...
router.post("/notes/insert", async (context) => {
...
router.post("/notes/update", async (context) => {
  if (!context.request.hasBody) {
    return context.response.status = 400;
  }
  const body = await context.request.body();
...
router.post("/notes/delete", async (context) => {
  if (!context.request.hasBody) {
    return context.response.status = 400;
  }
  const body = await context.request.body();
